...
* :region						AWS Region defaults to the region portion of the :availability_zone attribute. 
								Can use region to override if Amazon breaks the pattern of /(\w+-\w+-\d+)(\w+)/.match(availability_zone)
...
* :volume_id Volume whose snapshots to prune. If not specified, uses the current volume

===== Description

Prune all older snapshots other than the last number :snapshots_to_keep
